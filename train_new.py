...
class PhysicsEnhancedDetector(nn.Module):
    def __init__(self, use_physics=True):
        ...
        # Output dim = 2048 (ResNet) + 4 (Physics Vector) if enabled
    ...
    def forward(self, rgb_imgs, normals, intensities, obj_pts=None, shd_pts=None):
        # Branch 1: Extract standard visual features
        v_rgb = self.backbone(rgb_imgs)  # Shape: (Batch, 2048)

        if self.use_physics:
            # Branch 2: Compute physical residual vector
            v_phys = self.physics_layer(normals, intensities, obj_pts, shd_pts) # Shape: (Batch, 4)
            # Late Fusion: Concatenate visual and physical features
...
# ---------------------------------------------------------
# 3. Training Engine Loop (With tqdm)
# ---------------------------------------------------------
def train_one_epoch(model, dataloader, criterion, optimizer, epoch, total_epochs):
    ...
    # Wrap dataloader in tqdm progress bar
    progress_bar = tqdm(dataloader, desc=f"Epoch [{epoch}/{total_epochs}] Training", leave=True)
    ...
    for batch in progress_bar:
        ...
        # Update progress bar suffix with live loss and accuracy
...
# ---------------------------------------------------------
# 4. Validation Engine Loop (With tqdm)
# ---------------------------------------------------------
def validate(model, dataloader, criterion, epoch, total_epochs):
    model.eval()
    running_loss, correct_preds, total_samples = 0.0, 0, 0

    # Wrap validation dataloader in tqdm progress bar
    progress_bar = tqdm(dataloader, desc=f"Epoch [{epoch}/{total_epochs}] Validation", leave=True)
...
def custom_collate(batch):
    ...
    for o_pts, s_pts in zip(obj_pts_list, shd_pts_list):
        ...
        if k < max_k:
            ...
            shd_pts = torch.zeros((max_k, 2), dtype=torch.float32)
...
# ---------------------------------------------------------
def main():
    DATASET_PATH = "./dataset_root" # Path to your dataset directory
    BATCH_SIZE = 64
    EPOCHS = 5
    LEARNING_RATE = 1e-3
    MODEL_SAVE_PATH = "physics_enhanced_detector.pth"
